...
if [ ! $quiet ]; then
  echo "### Q-CHEM OPTIMIZED XYZ MAKER V1.0 ###"
  echo
...
if [ $1 ]; then
  ...
  if [ $otherout ] && [ -n $otherout ]; then
    output=$otherout
    echo "... using non default outputfile $output"
  else
    echo "Using default outputfile $output"
  fi

  for i in $file; do
    if [ $(cat $1 | grep -c "OPTIMIZATION CONVERGED") -eq 1 ]; then
      natoms=$(grep "NAtoms," $1 -A1 | tail -n1 | awk '{print $1}')
      nlines=$((natoms + 4))
      [ ! $quiet ] && echo "Found $natoms atoms..."

      if [ -e $output ]; then
        cp $output "$output"_bkup
      ...
      echo "Optimization not converged, exiting"
    fi
  done
  ...
  for i in *out; do
    ...
    if [ $(cat $1 | grep -c "OPTIMIZATION CONVERGED") -eq 1 ]; then
      natoms=$(grep "NAtoms," $i -A1 | tail -n1 | awk '{print $1}')
      nlines=$((natoms + 4))
      [ ! $quiet ] && echo "Found $natoms atoms..."
