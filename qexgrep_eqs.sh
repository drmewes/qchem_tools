...
for j in *out
        ...
        if [ -z "$nstates" ]
                then    nsinglets=$(tac $j | grep -m 1 ee_singlets | awk '{print $2}')
                        ntriplets=$(tac $j | grep -m 1 ee_triplets | awk '{print $2}')
                        if [ -n "$ntriplets" ] && [ -n "$nsinglets" ]
                                then nstates=$(echo "$nsinglets+$ntriplets" | bc)
                        elif [ -n "$nsinglets" ]
                                then nstates=$nsinglets
                        elif [ -n "$ntriplets" ]
                                then nstates=$ntriplets
                        else
                                echo "No ES found"
                                break
                        fi
        ...
        #echo "Going for $nstates states"
                ...
                do energy=$(tac $j | egrep "Excited state"[" "]\*"$i " -B 10 -m1| grep -e "Emission energy (PCM 1st order," | awk '{print $8}') && isref="EQS-REFERENCE"
